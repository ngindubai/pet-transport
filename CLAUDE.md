...
>
> **For any design / layout / CSS / template work, also read [DESIGN.md](DESIGN.md) — it is the second bible and governs how the site looks.**
...
- **Design system:** [DESIGN.md](DESIGN.md) — templates, CSS, the 6-variant anti-detection system
...
### Design work is its own discipline
- For any task about how the site **looks** (CSS, templates, layouts, components, colours, spacing, the homepage, the nav), read **[DESIGN.md](DESIGN.md)** before touching anything. It is the second bible.
- **The cascade rule:** this is a Hugo site. One template or shared-CSS change rewrites **thousands of live pages** on the next build. Always ask "how many pages does this touch?" before editing.
- **The anti-detection rule:** route pages render through **6 distinct layouts (A–E + legacy)**, each in its own CSS namespace (`.tpl-route-a` … `.tpl-route-e`). Keeping them structurally and visually **different** is what protects the site from Google's AI/spam detection. **Never make a change that causes variants to converge.** A change meant for one variant goes in that variant's namespace only.
- **Scoped change** (one variant, one component, the homepage) → make it, open a PR on a branch, summarise, stop.
- **Cascade change** (`main.css` global blocks, `baseof.html`, `header.html`, `footer.html`, `maincolors.css`, `hugo.toml` design params, the quote form) → describe the change and the page-count impact, get Gareth's explicit "yes", **then** open the PR.
- **Never edit vendor files** (`static/css/style.css`, `plugins.css`, Bootstrap, `template-source/`). Override in `site/assets/css/main.css`, which loads last.
- **Never edit `site/public/`** — it is gitignored build output.
...
- **Any cascade design change** (shared CSS / base template / nav / footer / colour / font that hits every page — see DESIGN.md)
...
│   ├── layouts/                   # Hugo templates (see DESIGN.md for the full map)
│   ├── assets/                    # css/main.css + js/main.js (Hugo pipeline — editable design)
│   ├── static/                    # CSS, JS, images, fonts (vendor template — do not edit vendor CSS/JS)
...
├── CLAUDE.md                      # THIS FILE
├── DESIGN.md                      # Design/template/CSS bible (read for any visual work)
...
### When the session is about design
1. Read [DESIGN.md](DESIGN.md) before touching any template or CSS.
2. Identify the file(s) and the CSS namespace, and state the page-count impact.
3. Scoped change → PR on a branch, summarise, stop. Cascade change → describe + get explicit "yes" first.
4. Never converge the 6 route variants. Never edit vendor CSS or `site/public/`.
...
6. **Design edits follow the same PR-on-a-branch flow** (e.g. `design/...`). See DESIGN.md for scoped-vs-cascade rules.
...
- Editable design lives in `site/assets/css/main.css` and `site/layouts/`. Vendor CSS/JS in `site/static/` is not edited. See DESIGN.md.
...
1. Re-read this file (and [DESIGN.md](DESIGN.md) for visual work).
