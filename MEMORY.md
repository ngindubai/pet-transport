...
- **Routes built:** 5,242 of ~37,830 country pairs (~13.9%). True on-disk count (5,232 in `routes/` + 10 in `pet-transport/`).
- **Blog articles:** 413
- **Total .md source files:** 6,028 (build_state.json `total_site_pages`). Full deployed page total, including Hugo taxonomy, verified from live sitemap.xml after a build.
- **Phase 7 progress:** Chunks 1-25 complete. **Chunk 26 is next** (Template D, Tier A). The 90-route P1 priority matrix is fully built; 227 score-7+ Tier A pairs remain after Chunk 25 (China, India, Malaysia, Thailand, Philippines, Vietnam, Ireland, NZ corridors now done).
