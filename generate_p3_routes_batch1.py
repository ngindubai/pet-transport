"""
Phase 3 / Task 3.3 Batch 1: P3 Route Page Generation
Generates Hugo markdown route pages for the first 50 high-priority P3 routes.
Covers: P3 x P1, P3 x P2, and high-value P3 x P3 pairs (both directions).
...
# ============================================================
# COUNTRY REGISTRY
# JSON key -> (display name, ISO alpha2, url-slug)
# ============================================================
COUNTRY_REGISTRY = {
    # P1
    ...
    "New_Zealand":     ("New Zealand",           "NZ", "new-zealand"),
    ...
    "Czech_Republic":  ("Czech Republic",        "CZ", "czech-republic"),
...
ORIGIN_HUB_URLS = {
    ...
    "australia":            "/pet-transport/origins/pet-export-guide-shipping-from-australia/",
    "canada":               "/pet-transport/origins/pet-export-guide-shipping-from-canada/",
    "germany":              "/pet-transport/origins/pet-export-guide-shipping-from-germany/",
    "france":               "/pet-transport/origins/pet-export-guide-shipping-from-france/",
    "singapore":            "/pet-transport/origins/pet-export-guide-shipping-from-singapore/",
    "hong-kong":            "/pet-transport/origins/pet-export-guide-shipping-from-hong-kong/",
    ...
    "malaysia":             "/pet-transport/origins/shipping-your-pet-from-malaysia-export-guide/",
    "india":                "/pet-transport/origins/shipping-your-pet-from-india-export-guide/",
    "portugal":             "/pet-transport/origins/shipping-your-pet-from-portugal-export-guide/",
    "netherlands":          "/pet-transport/origins/shipping-your-pet-from-netherlands-export-guide/",
    "italy":                "/pet-transport/origins/shipping-your-pet-from-italy-export-guide/",
    "denmark":              "/pet-transport/origins/shipping-your-pet-from-denmark-export-guide/",
    "mexico":               "/pet-transport/origins/shipping-your-pet-from-mexico-export-guide/",
    "brazil":               "/pet-transport/origins/shipping-your-pet-from-brazil-export-guide/",
    "switzerland":          "/pet-transport/origins/shipping-your-pet-from-switzerland-export-guide/",
    ...
    "austria":              "/pet-transport/origins/austria-pet-export-guide/",
    "belgium":              "/pet-transport/origins/belgium-pet-export-guide/",
    "poland":               "/pet-transport/origins/poland-pet-export-guide/",
    "turkey":               "/pet-transport/origins/turkey-pet-export-guide/",
    "israel":               "/pet-transport/origins/israel-pet-export-guide/",
    "saudi-arabia":         "/pet-transport/origins/saudi-arabia-pet-export-guide/",
    "qatar":                "/pet-transport/origins/qatar-pet-export-guide/",
    "kuwait":               "/pet-transport/origins/kuwait-pet-export-guide/",
    "china":                "/pet-transport/origins/china-pet-export-guide/",
...
# ============================================================
# P3 BATCH 1 ROUTES — 50 high-priority pairs
# ============================================================
P3_ROUTES = [
    # Ireland corridors
    ("Ireland", "UK"),
...
AIRLINE_SUMMARY = {
    ...
    "Qatar Airways": ("Qatar Airways", "cargo_only",
        ...
        "Temperature embargoes apply during Gulf summer (June-September)."),
    "Air China": ("Air China", "cabin_and_cargo",
        "Small pets allowed in cabin on domestic China routes. "
        "International: cargo only via Air China Cargo. Strict crate size and documentation requirements. "
        "China-specific import/export permits required. Advance booking of cargo space essential."),
    "Vietnam Airlines": ("Vietnam Airlines", "cabin_and_cargo",
        "Small pets (under 5kg with carrier) allowed in cabin on some routes. "
        "Cargo: larger pets in hold. Advance booking required. "
        "Vietnam import/export documentation must be fully prepared before travel."),
    ...
    "Kenya Airways": ("Kenya Airways", "cabin_and_cargo",
        "Small pets allowed in cabin. "
        "Cargo: pets in pressurised hold via Kenya Airways Cargo. "
        "Connects Nairobi Jomo Kenyatta hub to European and African destinations. "
        "IATA-compliant crate required. Advance approval needed."),
    "EgyptAir": ("EgyptAir", "cabin_and_cargo",
        "Small pets allowed in cabin (up to 8kg with carrier). "
        "Cargo: larger pets via EgyptAir Cargo. Connects Cairo hub to Europe, Middle East, and Africa. "
        "Egypt import/export documentation required. Advance booking recommended."),
    "Aeroflot": ("Aeroflot", "cabin_and_cargo",
        "Small pets (up to 8kg) allowed in cabin. "
        "Cargo: larger pets in pressurised hold. Connects Moscow Sheremetyevo hub. "
        "Note: service availability subject to current route status."),
}
...
def get_airlines_for_route(origin_key, dest_key):
    ...
    if dest_code in ("US", "CA", "MX", "AR", "CL", "CO", "BR") or \
       origin_code in ("US", "CA", "MX", "AR", "CL", "CO", "BR"):
        add("Delta Air Lines")
    ...
    if origin_code in ("CN", "TW") or dest_code in ("CN", "TW"):
        add("Air China")
        add("Cathay Pacific")
        add("Korean Air")

    if origin_code in ("VN",) or dest_code in ("VN",):
        add("Vietnam Airlines")

    if origin_code in ("QA",) or dest_code in ("QA",):
        add("Qatar Airways")

    if origin_code in ("TR",) or dest_code in ("TR",):
        add("Turkish Airlines")

    if origin_code in ("EG",) or dest_code in ("EG",):
        add("EgyptAir")

    if origin_code in ("KE", "NG") or dest_code in ("KE", "NG"):
        add("Kenya Airways")
        add("Emirates")

    if origin_code in ("SA", "KW") or dest_code in ("SA", "KW"):
        add("Emirates")
        add("Etihad Airways")
    ...
    if isinstance(q, dict):
        if q.get("required") is True or q.get("mandatory") is True:
            ...
            if q.get("penalty_quarantine") or q.get("penalty_duration_days"):
                note += ". Penalty quarantine applies if documentation is non-compliant"
            if q.get("notes"):
                note = q["notes"]
...
def assess_route(dest_key):
    ...
    if isinstance(bans, dict):
        ...
        if banned:
            score += 1
            warnings.append("Breed restrictions apply at destination. Check if your dog is affected before booking.")
...
def build_timeline_steps(dest_key):
    ...
    if isinstance(ip, dict) and ip.get("required") is True:
        ...
        steps.append({
            "step": step_num,
            "action": f"Apply for import permit from {dest_name}",
...
def generate_sections(origin_key, dest_key, complexity):
    ...
    heading = heading_variants[hash(f"{origin_key}{dest_key}") % len(heading_variants)]

    body_parts = []
    if complexity in ("very_high",):
        body_parts.append(
    ...
    tips_heading = tips_heading_variants[hash(f"{dest_key}{origin_key}") % len(tips_heading_variants)]
...
def main():
    ...
    print(f"Generating P3 Batch 1 routes... {len(P3_ROUTES)} route pairs to process")

    for origin_key, dest_key in P3_ROUTES:
