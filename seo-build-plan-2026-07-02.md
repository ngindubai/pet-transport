...
## New finding SB3 - hero quarantine stat uses the unreliable bool (flagged, not fixed)

The route hero "stat" blocks (in `route-new-na.html`, `route-new-nb.html`, and the equivalent in the other variants) show quarantine using `.Params.quarantine_required`, the same sparse bool that A4 abandoned. On Australia routes (and any route lacking the bool) the hero currently displays "None / No quarantine required", which is wrong and a YMYL concern, and it now contradicts the correct A4 capsule on the same page. Recommend switching those hero stats to the same reliable `route_data.destination.import_requirements.quarantine` logic used in `route-quick-answer.html`. Not fixed yet: it touches the hero of all six variants and is a judgement call on wording per design. Awaiting Gareth's go-ahead. Model: Sonnet OK.
