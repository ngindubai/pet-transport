...
## New finding SB1 - JSON-LD double-escaping (found during Block 2 verification)

While verifying the A5 author schema with a real Hugo build (installed via the Go module proxy, since the release binary is egress-blocked), I found a pre-existing sitewide bug: any `{{ value | jsonify }}` placed inside a `<script type="application/ld+json">` block gets JSON-escaped a second time by Go's html/template context-aware escaping, so values rendered as `"\"How long...\""` (a quoted string containing literal quote characters). This broke the values of FAQPage, HowTo, and Article schema, plus author names, on essentially every page. It defeats FAQ and HowTo rich-result eligibility, which is central to the schema and AI-Overview goal (audit Story 2). Gareth chose to fix all occurrences now.

Fix: append `| safeJS` after `jsonify` in every JSON-LD script (jsonify does the JSON encoding, safeJS stops the template re-encoding it). The raw `/index.json` search index is not inside a `<script>`, so it was correct already and left unchanged. Verified against a full Hugo build: route FAQ/HowTo, blog Article/author, and country/airline/breed/origin FAQ all now emit clean single-quoted JSON, and every JSON-LD block parses.
...
## New finding SB6 - raw Python dict strings in route data (FIXED, from live Prompt 4)

Live verification (Prompt 4) spotted `{'required': False}` rendering literally in the Oman-to-Austria export-permit field. Investigating: two fields stored raw Python dict literals instead of readable text: `export_permit` on 2,826 routes and `health_certificate` on 2,474 routes (values like `{'required': True, 'issued_by': 'SENASA Peru'}`). These rendered verbatim in the requirements tables across every variant. Fixed by parsing each dict (ast.literal_eval) and writing readable text: `{'required': False}` to "Not required"; `{'required': True}` to "Required"; with an issuer to "Required. Issued by X."; with a name to "{name}. Issued by X." 5,300 field instances fixed across 3,161 route files; already-readable prose values were left untouched; zero raw dict strings remain in the build. Only route files were affected (origins/countries were clean). This is data normalisation, not content generation. Model: Sonnet OK, build-verified.

## A5b done - visible byline on all route variants (FIXED, from live Prompt 4)

Prompt 4 confirmed the byline showed on variant A and country pages but not on the other route variants (germany-to-australia is variant C). This was the A5b gap. Fixed by adding `{{ partial "verified-date.html" . }}` (which renders the "Written and reviewed by [specialist]" byline plus the "Regulations verified" freshness stamp and dateModified schema) right after the quick-answer capsule in `route-new-nb/nc/nd/ne/nl.html`. All six variants now show the byline and freshness stamp. Verified: germany-to-australia now renders "Written and reviewed by Marcus Webb, Senior Pet Relocation Consultant". Model: Sonnet OK, build-verified.

## Non-issues confirmed by Prompt 4 (no action)

- FAQ and HowTo not surfaced in the Rich Results Test: expected, not a bug. Google deprecated FAQ/HowTo rich results for non-government/health pages in 2023. The JSON-LD is clean and valid (SB1 fix confirmed: no stray backslashes) and still useful for AI engines and general parsing, so it is kept.
- Header search box appears in the collapsed nav on narrower screens rather than always-visible in the header bar. It is functional and redirects correctly. Making it persistently visible is a design choice, flagged for Gareth, not auto-changed.

## New finding SB5 - route <title> ignored the title field sitewide (FIXED, from GSC Prompt 2)

Doing the A10 title work (GSC Prompt 2 data), I found that `routes/single.html` defined the `<title>` as `.Params.seo.title | default (printf "Pet Transport from %s to %s: Guide ...")`. Route pages have no `seo.title`, so every one of the 6,094 routes rendered the generic "Pet Transport from X to Y: Guide | Pet Transport Global" title and the carefully written `title:` field (for example "Pet Transport UK to Australia: Titre Test, Quarantine and the 6-Month Timeline") was never used in the `<title>` at all. That suppressed CTR across the entire route section, which is exactly what GSC showed. Fixed by changing the define to `.Params.seo.title | default .Title | default (printf ...)`, so the real `title:` field renders (the generic printf remains only as a last-resort fallback). Verified in the build: uk-to-australia now renders its full title, and every route uses its own. This is a sitewide win beyond the individual pages tuned. Model: Sonnet OK, build-verified.

## A10 done (data-driven, from GSC Prompt 2)

GSC Performance data (last 3 months) showed the low-CTR, position-5-20 pages were dominated by country hub pages (Sweden, South Korea, Turkey, etc., high impressions and ~0% CTR) plus a set of route pages at position 5-9. Actions: (1) SB5 above unblocked all route titles. (2) Rewrote titles and meta descriptions for 20 country pages from their real data, leading with "Pet Transport to {Country}" (matching the top query) and an accurate universal hook (No Quarantine, N-Day Quarantine, Import Permit, EU Rules); titre is stated only as "from some origins" because it is origin-conditional, and EU permit claims were dropped as origin-conditional. Country `<title>` renders from `seo.title`, so the new title was written there; the meta description renders from the top-level `description:`, which was updated; descriptions trimmed to ~120-160 chars. (3) Rewrote the 8 weak route titles (qatar-to-germany, germany-to-qatar, sri-lanka-to-australia, germany-to-turkey, australia-to-china, canada-to-india, australia-to-india, canada-to-china) that had generic "Requirements & Guide" titles, into fact-led titles from route data; the other listed routes already had strong titles. Files: 20 `countries/*.md`, 8 `routes/*.md`, `routes/single.html`. No em dashes, no banned vocabulary, build-verified. The 2 airline pages (flydubai, bulgaria-air) already had adequate titles and were left. Model: Sonnet OK.

## New finding SB4 - sitewide broken origin nav links (FIXED, from GSC Prompt 1)

Google Search Console (Prompt 1 result) reported 114 "Not found (404)" pages and 300 "Crawled, currently not indexed" (thin/duplicate). Investigating the 404s: the header mega-menu linked to four origin pages that do not exist, `/pet-transport/origins/pet-export-guide-shipping-from-{united-kingdom,united-states,australia,united-arab-emirates}/`, while the real pages are the plain slugs `/pet-transport/origins/{united-kingdom,united-states,australia,united-arab-emirates}/`. Because these were in the header, they 404'd on every one of the ~6,900 pages, so Google crawled them sitewide. Fixed the four links in `site/layouts/partials/header.html`. Then ran a full broken-internal-link audit over all 6,897 built pages: zero broken internal links remain. The 300 "crawled, not indexed" pages are the thin/duplicate signal that A1 de-duplication (deployed this session) directly targets, so that should improve on recrawl. The remaining 404s are almost certainly stale URLs from earlier route renames/deletions that are no longer linked anywhere and will drop out of the index; identifying them precisely needs the GSC 404 export (follow-up browser prompt). Model: Sonnet OK.

## Changes made and why (continued - Phase 6: A9, A10)

- **A10 - targeted title/meta tuning: already done where it matters.** Checked all 15 high-volume routes from `llms.txt` (UK-Australia, USA-UK, UAE-UK, Germany-Australia, etc.). Every one already has a strong, fact-rich, non-generic title and description (for example "Pet Transport UK to Australia: Titre Test, Quarantine and the 6-Month Timeline"), so there was no headless tuning to do on the routes that matter. 1,188 of 6,094 routes still use the generic "Shipping Dogs & Cats from X to Y" title; these are the long tail, which the audit said to leave on the template. Deciding which long-tail routes are gaining impressions and worth hand-tuning needs Google Search Console data. Semrush was attempted but the account has no API units, and the GSC verification code in `hugo.toml` is empty, so this becomes a browser task (see `BROWSER-PROMPTS.md`). No code change. Model: Sonnet OK; the remaining step is data-driven/manual.

- **A9 - quarterly data-refresh routine: built.** New `refresh_audit.py` scans every route, groups by destination country, reads each route's verified `date`, and prints a worklist ranked by staleness x routes affected, with the official source for each destination and the sources on record in `data/govt_import_regulations.json`. It changes no content; it tells the reviewer what to re-check and where. New `REFRESH-ROUTINE.md` documents the quarterly process (generate worklist, re-check against the authority via the browser prompts, update only what changed with a human sign-off, bump the verified date, log it). Verified: the script runs and produces a sensible ranked table (all data currently verified ~2026-06-04, nothing past the 90-day stale threshold yet). The regulatory re-checking itself is deliberately manual (YMYL); the browser prompts draft the comparison. Model: Sonnet OK to build and run; human sign-off on any regulatory change.

## Changes made and why (continued - Block 3: A1)

- **A1 - route-prose de-duplication (template composition, all variants).** Approach decision (Gareth): adopt template composition and roll out fully, instead of the plan's per-file `.md` rewrite. Rationale: it de-duplicates every route at once through the render layer, touches no content files, uses no bulk-generation script (so it respects CLAUDE.md), and is fully build-verifiable. Grounding: 4 middle-paragraph variants covered 91% of route overviews, and the "Microchip first, then vaccinate..." section body was identical on 4,790 routes. Two new partials compose prose from data-keyed sentence pools selected by a deterministic per-route seed (`hash.FNV32a` of the slug), so two routes only read alike if their data and seed coincide. `site/layouts/partials/routes/route-overview.html` outputs a two-paragraph overview from opener/complexity+timeline/quarantine/titre/closer pools (openers and closers carry no facts; factual clauses use the same YMYL-safe quarantine and titre logic as A4). `site/layouts/partials/routes/route-sections.html` `return`s a slice of {heading, body} dicts (so each variant keeps its own markup) with a complexity-keyed requirements section and a practical-tips section that picks four tips from a pool by seed; the titre-lab tip only appears when a titre test is relevant. Wired into all six variants: `route-new-na.html` renders the full composed overview as its Overview section; `nb/nc/nd/ne/nl` use the composed overview (plainified, truncated) for their hero subtitle; all six range over the composed sections. Both partials fall back to the stored prose when a route lacks `route_complexity` (12 routes). Verified with a full Hugo build: same-destination routes (for example six origins to Austria) now have distinct, accurate overviews and sections across all variants; the 4,790-route duplicate paragraph returns 0 on sampled pages; no em dashes, no banned vocabulary; build exits 0 with the page count unchanged (~6,909). A Hugo gotcha fixed during the work: a single top-level `return` with explicit tip picks replaced a double-`return`-in-`if` plus a `range`-reassignment that failed with "undefined variable". The stored `.md` overview/sections are now unused on complexity-bearing routes but are kept as the fallback; they can be stripped later if desired. Model: Opus designed the strategy, pools and selection logic and verified the sample; the mechanical wiring is Sonnet-level.

## Changes made and why (continued - Block 3)

- **A4 - answer-first capsule on route pages.** New partial `site/layouts/partials/routes/route-quick-answer.html`, included at the top of the main content column in all six active route variants (`route-new-na/nb/nc/nd/ne/nl.html`). Renders a short "In short:" summary built only from existing per-route data: preparation time (`estimated_timeline_weeks`), quarantine, and titre test. Content choice (Gareth): time + quarantine + titre note, no cost (no real prices in the data). YMYL-safe logic, hardened during build verification: quarantine reads the reliable `route_data.destination.import_requirements.quarantine` string (present on ~6,028 routes) rather than the sparse `quarantine_required` bool (only 150 routes) which had been defaulting Australia routes to "no quarantine"; it checks a clear "No/Not/None" prefix BEFORE extracting any day count, so penalty-day mentions in otherwise-no-quarantine strings (for example UK "Not required... penalty quarantine up to 120 days") are not misread as routine quarantine; it states a day count only when the string genuinely carries one, and stays silent on ambiguous strings (veterinary inspection, conditional). Titre distinguishes "Not required", "Required for/if" (conditional -> "may be required"), and plain "Required". Verified against a full Hugo build across many routes: Australia shows 10 days, India-Singapore 30 days, UK/EU show no quarantine, inspection-only routes omit the quarantine line. Model: Sonnet OK, but this one genuinely needed the build to catch the quarantine data bug. Note SB3 below.

## New finding SB3 - hero quarantine stat used the unreliable bool (FIXED)

The route hero and mini stats across all six variants showed quarantine using `.Params.quarantine_required`, the sparse bool that A4 abandoned, so on Australia routes they displayed "None / No quarantine required" (wrong, YMYL), contradicting the correct A4 capsule on the same page. Fixed by a new shared partial `site/layouts/partials/routes/quarantine-short.html` that returns a short reliable label ("None", "10 days", "Required", "Check guide") from the same `route_data.destination.import_requirements.quarantine` string and No-before-days logic as the capsule. Replaced all ten `quarantine_required`-based stat displays in `route-new-na/nb/nc/nd/ne/nl.html` (the full-string `{{ with .quarantine }}` displays were already correct and left alone). Some `nc` labels changed from "Days quarantine" to "Quarantine" so the label fits values like "None". Verified with a full Hugo build: Australia routes now show "10 days" or "Required" (both accurate, depending on how the route's data string is phrased), UK shows "None", Singapore "30 days"; no route shows a false "no quarantine". Model: Sonnet OK.
