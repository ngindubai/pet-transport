---
template_variant: "C"
title: "Pet Transport Portugal to Denmark | EU Pet Passport & Airline Comparison"
description: "Moving your pet from Portugal to Denmark? EU-EU travel with an EU pet passport. Compare TAP, SAS, Lufthansa, and other airlines. Check Denmark's breed ban list."
type: "routes"
layout: "single"
author: "Marcus Webb, Senior Pet Relocation Consultant, PetTransportGlobal"
slug: "portugal-to-denmark"
origin_name: "Portugal"
destination_name: "Denmark"
route_data:
  origin:
    code: "PT"
    country: "Portugal"
    export_requirements:
      export_permit: "Not required"
      health_certificate: "EU pet passport accepted for intra-EU travel to Denmark. No separate endorsement required."
  destination:
    code: "DK"
    country: "Denmark"
    import_requirements:
      microchip: "Required (ISO 11784/11785)."
      rabies_vaccination: "Required. 21-day wait after primary vaccination. Booster: no wait if administered before previous vaccination expires."
      titre_test: "Not required for EU-origin pets."
      quarantine: "No quarantine."
      import_permit: "Not required."
      health_certificate: "EU pet passport accepted. Danish Food and Veterinary Administration (FVST) governs compliance."
  airlines:
    - name: "TAP Air Portugal"
      type: "cabin_and_cargo"
      policy_summary: "Direct and indirect Lisbon-Copenhagen service. Small pets accepted in cabin (under 8kg including carrier). Larger pets via TAP Cargo. Good cabin pet acceptance record on European routes."
    - name: "SAS (Scandinavian Airlines)"
      type: "cabin_and_cargo"
      policy_summary: "Operates Lisbon-Copenhagen service. Denmark's partner national carrier. Small pets accepted in cabin. Familiarity with Danish pet import rules."
    - name: "Lufthansa"
      type: "cabin_and_cargo"
      policy_summary: "Via Frankfurt. Small pets in cabin. Larger pets via Lufthansa Cargo. Adds a connection but good pet handling at FRA."
    - name: "Air France"
      type: "cabin_and_cargo"
      policy_summary: "Via Paris CDG. Small pets in cabin. Indirect but a reliable option."
    - name: "Ryanair"
      type: "no_pets"
      policy_summary: "Does not accept pets. Not suitable for pet transport."
    - name: "easyJet"
      type: "no_pets"
      policy_summary: "Does not accept pets. Not suitable for pet transport."
  timeline_steps:
    - step: 1
      action: "Check EU pet passport is current"
      timing: "Confirm microchip and rabies vaccination are valid."
      responsible: "Your veterinarian"
    - step: 2
      action: "Confirm your dog is not on Denmark's banned breed list"
      timing: "Before booking -- Danish law bans 13 breeds."
      responsible: "You"
    - step: 3
      action: "Book flight with TAP or SAS; confirm pet space"
      timing: "1-2 weeks before travel."
      responsible: "You"
    - step: 4
      action: "Travel day"
      timing: "Present EU pet passport at check-in. No special entry process in Denmark."
      responsible: "You"
  cost_factors:
    - "Cabin pet fee (approximately EUR 40-80 with TAP or SAS)"
    - "Cargo fee for larger pets (EUR 120-400)"
    - "EU pet passport if not already issued"
    - "IATA-approved carrier or crate"
  key_warnings:
    - "Denmark bans 13 dog breeds including the Pit Bull Terrier and Tosa Inu. Banned breeds cannot legally be brought into Denmark even as EU residents. Check the full list before you book."
    - "Ryanair and easyJet do not accept pets. If you find a budget ticket, verify pet policy before purchase."
  route_complexity: "very_low"
  estimated_timeline_weeks: "1-4"
content:
  h1: "Pet Transport from Portugal to Denmark"
  overview: |
    Portugal to Denmark is intra-EU travel, which means your pet's EU pet passport is all you need on the documentation side. No titre test, no quarantine, no import permit. The 21-day wait after any primary rabies vaccination is the only significant timing constraint.

    The main choices are practical ones: which airline, and whether your pet travels in the cabin or as cargo. TAP Air Portugal and SAS both serve the Lisbon-Copenhagen route and accept pets in cabin.

    There is one important check before you do anything else: verify your dog is not on Denmark's prohibited breed list. Denmark bans 13 breeds under the Dog Act, and the ban applies to all dogs including those legally owned in Portugal.
  sections:
    - heading: "TAP Air Portugal vs SAS: which is better for your pet?"
      body: |
        Both TAP and SAS operate on the Lisbon-Copenhagen route and both accept pets in cabin. For most owners, the choice comes down to schedule and price.

        TAP is your home carrier and has good familiarity with cabin pet bookings from Lisbon. Their cabin pet policy allows animals under 8kg (including carrier) in the cabin. Larger animals travel as cargo.

        SAS brings its Scandinavian orientation: their staff at Copenhagen Airport are accustomed to pet arrivals. SAS also accepts small pets in cabin on this route. If you are settling into Denmark for the long term, some owners find SAS useful for loyalty programme purposes.

        Neither airline needs any extra documentation beyond the EU pet passport and the standard pet booking form.

        Budget carriers on this route -- Ryanair and easyJet in particular -- do not accept pets in any capacity. If someone recommends a budget flight for a pet owner, it is worth double-checking their assumptions.
    - heading: "Denmark's breed ban: what you need to know"
      body: |
        Denmark's Dog Act (Hundeloven) prohibits the import, ownership, and breeding of 13 dog breeds. The list includes: Pit Bull Terrier, Tosa Inu, American Staffordshire Terrier, Fila Brasileiro, Dogo Argentino, American Bulldog, Boerboel, Kangal, Central Asian Shepherd Dog, Caucasian Shepherd Dog, South Russian Ovcharka, Tornjak, and Sarplaninac.

        This applies to all dogs regardless of where they were born or how long the owner has lived in Portugal. Moving from Portugal to Denmark does not create an exemption.

        Mixed-breed dogs that appear to match a prohibited breed may also be assessed under Danish law. This is a real risk for certain rescue dogs.

        If your dog is one of these breeds or potentially one of them, seek legal advice before booking anything. There is no workaround.
    - heading: "What happens when you land in Copenhagen with a pet?"
      body: |
        Arriving at Copenhagen Airport (CPH) with a pet from Portugal is routine for EU residents. Present the EU pet passport at the border. A veterinary inspection may occur but is not always mandatory for intra-EU movements from low-risk countries.

        Danish authorities (FVST) can spot-check pet documentation at the border. Ensure your pet's EU passport is fully and correctly completed, with microchip number, vaccination dates, and veterinarian details all legible and consistent.

        There is no quarantine facility wait. If your pet's documentation is in order, they travel home with you.
faqs:
  - question: "Do I need anything other than an EU pet passport to bring my pet from Portugal to Denmark?"
    answer: "No. An EU pet passport is sufficient for a dog, cat, or ferret moving between Portugal and Denmark. The passport must show a valid microchip and a current rabies vaccination. No titre test, quarantine, or import permit is required."
  - question: "Is my dog breed allowed in Denmark?"
    answer: "Denmark bans 13 breeds under the Dog Act. Check the list: Pit Bull Terrier, Tosa Inu, American Staffordshire Terrier, Fila Brasileiro, Dogo Argentino, American Bulldog, Boerboel, Kangal, Central Asian Shepherd, Caucasian Shepherd, South Russian Ovcharka, Tornjak, and Sarplaninac. If your dog is on this list, it cannot legally enter Denmark."
  - question: "Can I take my cat in the cabin with TAP Air Portugal from Lisbon to Copenhagen?"
    answer: "Yes. TAP accepts cats and small dogs in the cabin on this route. The combined weight of pet plus carrier must be under 8kg. The carrier must fit under the seat in front of you. Reserve your pet's place when booking your own ticket."
  - question: "My dog's rabies vaccination expired. What do I do?"
    answer: "Book a booster with your vet as soon as possible. If your dog has had previous rabies vaccinations and the booster is given before the previous one fully lapses, no 21-day wait applies. If you have missed the window and the booster is given after expiry, a new 21-day wait applies. Your vet will know which situation applies."
  - question: "How much does it cost to bring a small dog from Lisbon to Copenhagen?"
    answer: "Budget EUR 40-80 for the cabin pet fee on TAP or SAS. Add the cost of an IATA-compliant soft carrier if you do not have one. For dogs travelling as cargo, fees range from EUR 120-400 depending on size. The total cost of the journey is usually under EUR 200 for a small dog."
links:
  sideways:
    - url: "/pet-transport/denmark-to-portugal/"
      text: "Pet Transport Denmark to Portugal"
  upward:
    - url: "/pet-transport/origins/portugal-pet-export-guide/"
      text: "Shipping from Portugal"
    - url: "/pet-transport/countries/denmark/"
      text: "Importing to Denmark"
    - url: "/pet-transport/airlines/tap-air-portugal/"
      text: "TAP Air Portugal pet policy"
    - url: "/pet-transport/airlines/sas/"
      text: "SAS pet policy"
---
