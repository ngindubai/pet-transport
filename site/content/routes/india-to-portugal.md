---
template_variant: "C"
title: "Pet Transport from India to Portugal | 2026 EU Entry via Lisbon"
description: "Pets from India to Portugal require FAVN titre test, 90-day wait, DAHD health cert. TAP Air Portugal direct to LIS. Costs, timelines and 2026 EU rules."
date: "2026-06-06"
type: "routes"
layout: "single"
author: "Marcus Webb, Senior Pet Relocation Consultant, PetTransportGlobal"
slug: "india-to-portugal"
origin_name: "India"
destination_name: "Portugal"
route_data:
  origin:
    code: "IN"
    country: "India"
    export_requirements:
      health_certificate: "DAHD-authorised vet health certificate"
      export_permit: "No export permit; DAHD cert required"
  destination:
    code: "PT"
    country: "Portugal"
    import_requirements:
      microchip: "ISO 11784/11785 microchip; must pre-date first rabies vaccination"
      rabies_vaccination: "Valid rabies vaccination; 21-day wait post-primary course before travel"
      titre_test: "Required. India not on EU listed countries. FAVN titre test at 0.5 IU/ml. Blood drawn 30+ days after vaccination. 90-day wait from passing result."
      quarantine: "No quarantine for compliant pets"
      import_permit: "No import permit required"
      health_certificate: "DAHD-endorsed certificate accepted by Portuguese DGAV (Directorate-General for Food and Veterinary Affairs)"
      breed_restrictions: "Portugal has a restricted breed list including Pit Bull Terrier, Rottweiler, Staffordshire Bull Terrier and others under Lei 9/2009. Owners of listed breeds need a special licence."
  airlines:
    - name: "TAP Air Portugal"
      type: "mixed"
      policy_summary: "LIS to DEL direct (introduced 2022). In-cabin for qualifying small pets; cargo for larger animals on India-Lisbon routes."
    - name: "Air India"
      type: "cargo_only"
      policy_summary: "DEL to LIS via connections. Live animals as cargo; contact Air India Cargo."
  timeline_steps:
    - step: 1
      action: "ISO microchip implanted and pre-dating vaccination"
      timing: "Before first rabies vaccination"
      responsible: "Vet"
    - step: 2
      action: "Primary rabies vaccination"
      timing: "At least 30 days before titre test blood draw"
      responsible: "Vet"
    - step: 3
      action: "FAVN titre test blood draw at EU-approved lab"
      timing: "30+ days post-vaccination"
      responsible: "Vet and EU-approved lab"
    - step: 4
      action: "Passing titre result received (0.5 IU/ml or above)"
      timing: "2-4 weeks for results"
      responsible: "Approved lab"
    - step: 5
      action: "90-day wait from passing result date"
      timing: "Three calendar months; fixed, cannot be shortened"
      responsible: "Owner"
    - step: 6
      action: "DAHD vet issues export health certificate"
      timing: "Within 10 days of departure"
      responsible: "DAHD vet"
    - step: 7
      action: "Book TAP or Air India cargo space"
      timing: "4-6 weeks before travel"
      responsible: "Owner or agent"
  cost_factors:
    - "FAVN titre test India: USD 200-400"
    - "DAHD health certificate: INR 2,000-5,000"
    - "Cargo India to Lisbon: USD 900-2,200"
    - "Professional relocation agent: USD 500-1,200"
  key_warnings:
    - "Portugal has a restricted dangerous dog list under Lei 9/2009. Owners of breeds on this list need specific licensing to keep them in Portugal, separate from the import process."
    - "The 90-day wait is mandatory and begins on the passing titre result date, not the vaccination date."
    - "TAP Air Portugal's DEL-LIS route is relatively new. Confirm live animal cargo acceptance and any seasonal restrictions when booking."
    - "DGAV border inspectors at Lisbon Airport are experienced with Indian-origin pet documentation but will scrutinise all date sequences carefully."
  route_complexity: "high"
  estimated_timeline_weeks: "20-28"

content:
  h1: "Pet Transport from India to Portugal"
  overview: |
    Portugal has become an increasingly popular destination for Indian professionals and families, and Lisbon's appeal as a European base makes this a route we handle with growing frequency. The regulatory path follows the standard EU non-listed country process: FAVN titre test, 90-day wait, DAHD health certificate. The total timeline from starting documentation to boarding is typically 20 to 28 weeks.

    TAP Air Portugal now operates a direct Delhi to Lisbon (LIS) service, which simplified the logistics side of this move considerably. Before that route launched, India to Portugal required connections through European hubs, adding complexity and transit time for pets. The direct service gives owners a more manageable journey for their animals.

    One specific point for Portugal: the country has its own restricted breed legislation under Lei 9/2009. Certain breeds require a special owner licence to be kept in Portugal. This is a domestic ownership requirement rather than a border restriction, but it is important to understand before moving a listed breed.
  sections:
    - heading: "At a glance: India to Portugal vs India to Italy vs India to Switzerland"
      body: |
        | Requirement | India to Portugal | India to Italy | India to Switzerland |
        |---|---|---|---|
        | Listed country | No (EU) | No (EU) | No (OSAV) |
        | FAVN titre test | Required | Required | Required |
        | 90-day wait | Yes | Yes | Yes |
        | Quarantine (compliant) | No | No | No |
        | Direct airline | TAP (LIS) | ITA/Air India (FCO/MXP) | Swiss (ZRH) |
        | Restricted breed law | Yes (Lei 9/2009) | No national ban | Cantonal rules |
        | Typical timeline | 20-28 weeks | 20-28 weeks | 20-28 weeks |

        The core regulatory process is the same for all three routes. What varies is the destination's domestic breed legislation. Portugal's Lei 9/2009 creates a specific list of breeds requiring special licences for ownership in the country: this includes Pit Bull Terrier, Rottweiler, Staffordshire Bull Terrier, American Staffordshire Terrier, Dogo Argentino, Fila Brasileiro and others. Owners of these breeds will not be refused at the border, but they must register and licence their dog with Portuguese authorities within a set period of arrival.

        Italy has no equivalent national legislation, making it somewhat more straightforward for owners of mixed or restricted-type breeds. Switzerland applies cantonal rules, meaning requirements vary by region.

        On the airline side, Portugal now benefits from TAP Air Portugal's direct Delhi service, giving it a clear logistical advantage over Italy for Indian owners who prefer direct routing. Italy's ITA Airways and Air India options are equally reliable, but the choice often comes down to flight schedules and cargo availability on specific dates.

    - heading: "Understanding Portugal's Lei 9/2009 restricted breed rules"
      body: |
        Lei 9/2009 (as amended) governs the ownership of what Portuguese law calls "potentially dangerous dogs." The list includes several breeds that are commonly kept as family pets in India and internationally. The breeds covered include, but are not limited to: Pit Bull Terrier, Rottweiler, Staffordshire Bull Terrier, American Staffordshire Terrier, Dogo Argentino, Fila Brasileiro, Tosa Inu, Dogue de Bordeaux, and certain shepherd breeds in some interpretations.

        If your dog is a listed breed, ownership in Portugal is legal but regulated. You must register the dog with the Junta de Freguesia (local parish council) within a defined period after arrival, hold third-party liability insurance, and meet housing and muzzle-in-public requirements. Your dog must also hold a valid SNIAC (national animal identification system) registration.

        This is separate from the EU import process. At the border, DGAV inspectors are checking EU import compliance (microchip, vaccination, titre test, health certificate). The Lei 9/2009 registration is a post-arrival domestic obligation. Failure to comply with it is a domestic legal matter, not an immigration issue, but it is important to understand and prepare for before the move.

        If you are unsure whether your dog's breed or mix falls under Lei 9/2009, seek advice from a Portuguese vet or legal adviser before travel.

    - heading: "The practical journey: TAP cargo from India to Lisbon"
      body: |
        TAP Air Portugal's Delhi (DEL) to Lisbon (LIS) direct service is the recommended option for most India-based owners. The route runs several times weekly, and TAP has a dedicated cargo team familiar with live animal handling. Small pets meeting TAP's cabin criteria can travel in-cabin; larger dogs travel in the climate-controlled cargo hold.

        As with any cargo booking, confirm pet space availability well in advance. TAP limits the number of animals per flight, and this is a relatively new service, so capacity and handling protocols may differ slightly from TAP's more established European routes. Your agent can coordinate directly with TAP Cargo.

        At Lisbon Airport, DGAV inspectors handle live animal arrivals at a dedicated veterinary border post. They will check the microchip, the titre test report (original, not photocopy), vaccination records, and health certificate. Bring all originals in carry-on luggage. Processing typically takes one to two hours for Indian-origin pets.

        Air India via connections remains an option for owners who cannot travel on TAP's flight schedule, or if TAP cargo is full. Mumbai-based owners may find Air India's connections (typically via DEL or through European hubs) more convenient than routing through Delhi for TAP.

        Budget USD 900-2,200 for air cargo, USD 200-400 for the FAVN titre test, and INR 2,000-5,000 for the DAHD health certificate. With an experienced agent (USD 500-1,200), total costs for a medium dog run approximately USD 1,800-4,000.

faqs:
  - question: "What breeds are on Portugal's restricted dog list under Lei 9/2009?"
    answer: "The list includes Pit Bull Terrier, Rottweiler, Staffordshire Bull Terrier, American Staffordshire Terrier, Dogo Argentino, Fila Brasileiro, Tosa Inu, and Dogue de Bordeaux among others. These breeds can be brought into Portugal but require post-arrival licensing and registration with local authorities."
  - question: "Does Portugal require a titre test for pets from India?"
    answer: "Yes. India is not on the EU's listed countries. The FAVN titre test (0.5 IU/ml minimum, from an EU-approved lab) is mandatory, followed by a 90-day wait from the date of the passing result."
  - question: "Does TAP Air Portugal accept pets from India in the cabin?"
    answer: "TAP allows small pets in the cabin on eligible routes. For the Delhi-Lisbon direct service, confirm TAP's current cabin pet policy and weight limits when booking, as policies on specific long-haul routes can differ from short-haul rules."
  - question: "Is there quarantine when arriving in Portugal from India?"
    answer: "No quarantine applies for compliant pets (passing titre test, correct microchip, valid vaccination, health certificate within 10 days). Quarantine would only apply if documentation is deficient or the titre test has not passed."
  - question: "How long does the whole India to Portugal process take?"
    answer: "From starting the titre test process (rabies vaccination, then blood draw 30 days later, then results, then 90-day wait), the minimum is approximately 20 to 28 weeks. Start at least six months before your planned move date."

links:
  sideways:
    - url: "/pet-transport/india-to-italy/"
      text: "Pet transport from India to Italy"
    - url: "/pet-transport/india-to-switzerland/"
      text: "Pet transport from India to Switzerland"
    - url: "/pet-transport/india-to-netherlands/"
      text: "Pet transport from India to the Netherlands"
  upward:
    - url: "/pet-transport/countries/india/"
      text: "Pet transport from India"
    - url: "/pet-transport/countries/portugal/"
      text: "Pet transport to Portugal"
---
