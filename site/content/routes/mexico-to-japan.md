---
title: "Pet Transport Mexico to Japan: AQS Process and 180-Day Rule for 2026"
description: "Taking your pet from Mexico to Japan? Japan AQS requires two vaccinations, titre test at a designated lab, and 180-day wait. Complete 2026 guide."
date: "2026-06-06"
type: "routes"
layout: "single"
template_variant: "D"
author: "Marcus Webb, Senior Pet Relocation Consultant, PetTransportGlobal"
slug: "mexico-to-japan"
origin_name: "Mexico"
destination_name: "Japan"

route_data:
  origin:
    code: "MX"
    country: "Mexico"
    export_requirements:
      health_certificate: "SENASICA health certificate issued by registered vet and endorsed by SENASICA"
      export_permit: "SENASICA export documentation required"
  destination:
    code: "JP"
    country: "Japan"
    import_requirements:
      microchip: "ISO 11784/11785 microchip required before first vaccination"
      rabies_vaccination: "Two rabies vaccinations: second at least 30 days after first"
      titre_test: "FAVN titre test at AQS-designated laboratory (0.5 IU/ml minimum), blood 30+ days after second vaccination"
      quarantine: "180-day wait from blood draw; minimum 12-hour AQS inspection at Narita/Haneda if documents correct"
      import_permit: "Advance notification to Japan AQS at least 40 days before arrival"
      health_certificate: "AQS-format health certificate within 10 days of travel, SENASICA-endorsed"
      breed_restrictions: "No national breed ban in Japan; some airline restrictions on brachycephalic breeds"

  airlines:
    - name: "Aeromexico"
      type: "cargo_only"
      policy_summary: "MEX to NRT direct service available; Aeromexico cargo accepts live animals with full AQS documentation"
    - name: "Japan Airlines"
      type: "cargo_only"
      policy_summary: "MEX to NRT via JAL Cargo connections; JAL is experienced with AQS requirements"
    - name: "ANA Cargo"
      type: "cargo_only"
      policy_summary: "MEX to NRT via US hubs; ANA Cargo handles live animals at Narita with AQS familiarity"

  timeline_steps:
    - step: 1
      action: "Microchip implanted"
      timing: "Before any vaccination"
      responsible: "Accredited Mexican vet"
    - step: 2
      action: "First rabies vaccination"
      timing: "After microchip confirmed"
      responsible: "Accredited Mexican vet"
    - step: 3
      action: "Second rabies vaccination"
      timing: "At least 30 days after first"
      responsible: "Accredited Mexican vet"
    - step: 4
      action: "FAVN titre test at AQS-designated laboratory"
      timing: "At least 30 days after second vaccination"
      responsible: "Mexican vet, AQS-designated lab only"
    - step: 5
      action: "180-day wait from blood draw date"
      timing: "Count starts on the blood draw date"
      responsible: "Owner to track precisely"
    - step: 6
      action: "Advance AQS notification submitted"
      timing: "At least 40 days before arrival"
      responsible: "Pet transport agent"
    - step: 7
      action: "SENASICA health certificate in AQS format"
      timing: "Within 10 days of travel"
      responsible: "SENASICA-registered vet"

  cost_factors:
    - "Two rabies vaccinations: USD 60-120"
    - "FAVN titre test at AQS-designated lab in Mexico: USD 120-200"
    - "SENASICA certificate and endorsement: USD 80-200"
    - "Cargo MEX to NRT: USD 1,400-3,000"
    - "AQS inspection fee at Narita: approximately JPY 7,000-15,000"
    - "IATA crate: USD 100-400"

  key_warnings:
    - "The AQS-designated lab requirement is absolute; results from non-designated labs are not accepted."
    - "The 180-day wait from blood draw is non-negotiable. Early arrival means quarantine for the remaining days."
    - "Advance AQS notification at least 40 days before arrival: missing this deadline means your pet cannot land."
    - "SENASICA endorsement can take up to a week; allow sufficient time before the travel date."

  route_complexity: "high"
  estimated_timeline_weeks: "36-40"

content:
  h1: "Pet Transport Mexico to Japan: 2026 AQS Guide"
  overview: "Mexico to Japan is one of the more complex international pet routes. Japan's Animal Quarantine Service classifies Mexico as a non-listed country, requiring two rabies vaccinations, an FAVN titre test at an AQS-designated laboratory, a 180-day wait from the blood draw, and an advance notification to AQS at least 40 days before arrival. The whole process takes nine months or more from start to travel."

  sections:
    - heading: "What makes the Japan AQS process so different from the EU process?"
      body: |
        When families who have previously moved pets to Europe encounter the Japan AQS requirements, the immediate reaction is often surprise at the difference in scope. The EU process requires one vaccination and a titre test, with a 90-day wait. Japan requires two vaccinations with a gap between them, a titre test at a specifically designated laboratory (not just any approved lab), a 180-day wait, and a mandatory advance notification 40 days before arrival. That is a substantially larger undertaking.

        The reason is straightforward: Japan has been rabies-free since 1957. The AQS takes every possible measure to prevent reintroduction of the disease. For non-listed countries like Mexico, where rabies is endemic, the full protocol applies without exception.

        Mexico is a non-listed country for Japan AQS purposes, meaning the most demanding import pathway applies. The two-vaccination requirement (first and second vaccination at least 30 days apart) is specific to Japan. After the second vaccination, you wait another 30 days, then draw blood for the titre test. The blood must go to an AQS-designated lab. Japan publishes its approved laboratory list on the AQS website (maff.go.jp); in Mexico, confirm which labs are currently on the list before drawing blood.

        Once the satisfactory titre result is in (0.5 IU/ml or above), the 180-day wait begins from the blood draw date. Travel before this date and your pet faces quarantine at the AQS facility for the remaining days. The 40-day advance notification must be submitted through the formal AQS pre-registration system well before your travel date.

    - heading: "Aeromexico to Tokyo: how does the cargo routing work?"
      body: |
        Aeromexico operates a direct MEX-NRT (Mexico City to Tokyo Narita) service, which simplifies the cargo routing compared to many other Latin American origins. A direct flight means fewer handling events for your pet and a clearer chain of custody for the documentation package.

        Aeromexico Cargo accepts live animals with the full AQS documentation package. Confirm with their cargo desk that live animals are accepted on the specific MEX-NRT service for your pet's dimensions and weight. Some aircraft configurations on the route have limited hold space for large crates.

        Japan Airlines (JAL) and ANA also serve MEX-NRT, in some cases via US hubs. JAL Cargo is particularly experienced with AQS documentation requirements for Mexico-origin pets, as there is a significant Japan-Mexico business and expatriate community.

        Cargo costs from MEX to NRT typically run USD 1,400-3,000. The trans-Pacific flight is long: approximately 14 hours direct. Your pet travels in the pressurised hold throughout. IATA-approved crates are mandatory, and the crate must give your pet room to stand, turn, and lie down naturally.

    - heading: "What does the AQS inspection at Narita look like?"
      body: |
        On arrival at Narita (or Haneda), your pet is transferred to the AQS inspection facility. This is a government-run animal inspection centre separate from the main airport terminal. The facility has kennels for dogs and separate areas for cats.

        For a Mexican-origin pet with all documentation correct and the 180-day wait confirmed complete, the minimum inspection period is approximately 12 hours. During this time, AQS vets check the microchip number against all records, verify each step in the vaccination and titre test sequence, confirm the dates, and conduct a health inspection of the animal.

        Once the inspection is complete and everything is confirmed in order, your pet is released and can be collected from the facility. Your transport agent's Japan partner handles the coordination with the AQS facility and the collection logistics.

        If any documentation error is found during the inspection, or if the 180-day count is found to be incomplete, your pet's stay at the facility extends to cover the remaining wait period. The facility is functional: your pet will be fed, watered, and monitored, but it is not a comfortable home environment. Getting the documentation right before departure is everything on this route.

faqs:
  - question: "Is the Aeromexico MEX-NRT direct flight the best option for my pet?"
    answer: "For most pets from Mexico City, the direct Aeromexico service is the most straightforward option: fewer handling events and a clear routing. For very large pets in heavy crates, confirm with Aeromexico Cargo whether your specific crate dimensions are accepted. JAL is a strong alternative with good AQS familiarity."
  - question: "How do I find AQS-designated labs in Mexico?"
    answer: "Japan AQS publishes its designated laboratory list on the maff.go.jp website. Search for Mexico on the list. If no Mexican labs are currently listed, you will need to send the blood sample to an AQS-designated lab in another country: the United States, Canada, or an approved European lab. Confirm the current list before drawing blood."
  - question: "Can the advance AQS notification be submitted by me, or does it need an agent?"
    answer: "The advance notification form is available in English on the Japan AQS website. It can technically be submitted by the owner. However, the form requires detailed information about your pet's vaccination and titre test records, the arrival flight, and your Japan contact details. Most families find it practical to have a transport agent handle the submission to ensure accuracy."
  - question: "What happens to my pet if I send the blood to a non-designated lab by mistake?"
    answer: "The titre test result from a non-designated lab is invalid for AQS purposes. It does not count toward the 180-day wait. You must restart the vaccination process from the beginning. This can add 6 or more months to the timeline and is one of the most costly mistakes made on this route."
  - question: "Can cats travel to Japan from Mexico on the same AQS process as dogs?"
    answer: "Yes. Cats from Mexico follow exactly the same Japan AQS process as dogs: microchip, two vaccinations with 30-day gap, titre test at AQS-designated lab, 180-day wait, advance notification, and AQS-format health certificate. The process is identical for both species."

links:
  sideways:
    - url: "/pet-transport/mexico-to-south-korea/"
      text: "Pet transport Mexico to South Korea"
    - url: "/pet-transport/brazil-to-japan/"
      text: "Pet transport Brazil to Japan"
    - url: "/pet-transport/malaysia-to-japan/"
      text: "Pet transport Malaysia to Japan"
  upward:
    - url: "/pet-transport/countries/japan/"
      text: "Importing pets to Japan: country guide"
    - url: "/pet-transport/countries/mexico/"
      text: "Exporting pets from Mexico: country guide"
---
